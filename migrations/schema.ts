import { pgTable, serial, text, integer, boolean, numeric, timestamp, index, foreignKey, unique, varchar, json } from "drizzle-orm/pg-core"
...
export const vendorProductMappings = pgTable("vendor_product_mappings", {
	...
	index("vendor_sku_idx").using("btree", table.supportedVendorId.asc().nullsLast().op("int4_ops"), table.vendorSku.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const supportedVendors = pgTable("supported_vendors", {
	...
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow().notNull(),
	vendorShortCode: text("vendor_short_code"),
	imageQuality: text("image_quality").default('high'),
}, (table) => [
	unique("supported_vendors_name_unique").on(table.name),
	unique("supported_vendors_vendor_slug_unique").on(table.vendorSlug),
...
export const vendorInventory = pgTable("vendor_inventory", {
	...
	index("vendor_sku_inventory_idx").using("btree", table.supportedVendorId.asc().nullsLast().op("int4_ops"), table.vendorSku.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.supportedVendorId],
			foreignColumns: [supportedVendors.id],
