import { pgTable, serial, text, integer, boolean, numeric, timestamp, index, foreignKey, unique, varchar, json, check } from "drizzle-orm/pg-core"
...
export const vendorProductMappings = pgTable("vendor_product_mappings", {
	...
	index("vendor_sku_idx").using("btree", table.supportedVendorId.asc().nullsLast().op("int4_ops"), table.vendorSku.asc().nullsLast().op("text_ops")),
	foreignKey({
