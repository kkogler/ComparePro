...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/zoho/callback', async (req, res) => {
    try {
      ...
      res.send(`
        ...
        <hr>
        <p><small>This page is generated by your application OAuth callback endpoint.</small></p>
      `);
    ...
    try {
      ...
      try {
        const portalUrl = await zohoBilling.generateCustomerPortalUrl(
          testResults.tests.createCustomer?.customerId || 'test-customer',
          'http://localhost:3000/subscription/portal'
        );
      ...
      res.json({
        ...
        message: proxyConfigured && ipResponse.trim() === process.env.PROXY_HOST
          ...
          : proxyConfigured 
            ...
            : 'ℹ️ No proxy configured. Using local IP.'
      });
    } catch (error: any) {
  ...
  if (proxyConfigured) {
    ...
    console.log('ℹ️  STARTUP: No proxy configured - using local IP');
  }
